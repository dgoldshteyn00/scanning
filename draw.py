...
def scanline_convert(polygons, i, screen, zbuffer):
    ...
    dx0 = (top[0] - low[0]) / (top[1] - low[1])
    if mid[1] - low[1] != 0:
        dx1 = (mid[0] - low[0]) / (mid[1] - low[1])
    else:
        dx1 = None
    if top[1] - mid[1] != 0:
    ...
    dz0 = (top[2] - low[2]) / (top[1] - low[1])
    if mid[1] - low[1] != 0:
        dz1 = (mid[2] - low[2]) / (mid[1] - low[1])
    else:
        dz1 = None

    while y < mid[1]:
        if (x1 <= top[0] or x1 <= mid[0] or x1 <= low[0]) and (x0 >= top[0] or x0 >= mid[0] or x0 >= low[0]):
